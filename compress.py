...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            ...
            # else:
            #     fi = fitness_coco(np.array(results).reshape(1, -1))  # weighted combination of [P, R, mAP@.5, mAP@.5-.95]
            if fi > best_fitness:
                best_fitness = fi
...
def main(opt, callbacks=Callbacks()):
    ...
    # Resume
    if opt.resume and not check_comet_resume(opt) and not opt.evolve:  # resume an interrupted run
